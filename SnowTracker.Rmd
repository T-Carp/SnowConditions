...
### Incremental refresh using pins
```{r, message=FALSE}
obs <- pin_get("hist_obs")
...
### Convert from metric system (Declare function for switch statement)
```{r}
...
### Shiny Workspace (Add static line for freezing level on Temp Chart)
```{r, message=FALSE}
ui <- dashboardPage(
...
data_filter <- function(selection) {
  ...
  data <- reactive({data_filter(as.numeric(input$select))})
