  ...
  dashboardBody(
    fluidRow(
          ...
          valueBox(
            ...
            width = 3),
          valueBox(
            "Snow","Snow in last 7 days",
            color = "light-blue",
            icon = icon("snowflake"),
            width = 3),
          valueBox(
            "Test","Current Snow Density",
            color = "aqua",
            icon = icon("thumbs-up"),
            width = 3),
           valueBox(
            "Test","Precipitation Last 7 days",
            color = "teal",
            icon = icon("tint"),
            width = 3)
          ),
...
server <- function(input, output) {
  ...
  output$snowdepth <- renderPlotly({
     ...
     p <- ggplot(data,aes(date,snwd))+
        ...
        ggtitle("Snow Depth by Weather Station (Inches)")
     ggplotly(p)
  })
  ...
  output$map <- renderLeaflet({
    ...
    zoom_lng <- mean(data$longitude)
    zoom_lat <- mean(data$latitude)
...
output$temp <- renderPlotly({
 ...
 f <- ggplot(temp_final,aes(date,temp))+
        ...
        scale_x_date(limits = c(input$date[1], input$date[2]))+
        theme(legend.position="none")+
        ggtitle("Temperature by Mountain (Farenheit)")
...
output$prcp <- renderPlotly({
 ...
 q <- ggplot(prcp_final,aes(date,prcp))+
        ...
        scale_x_date(limits = c(input$date[1], input$date[2]))+
        theme(legend.position="none")+
        ggtitle("Percipitation by Weather Station (Inches)")
     ggplotly(q)
